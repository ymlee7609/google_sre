...
If the service has exceeded its [error budget](https://sre.google/sre-book/embracing-risk/) for the preceding four-week window, we will halt all changes and releases other than P0[^1] issues or security fixes until the service is back within its SLO.
...
If a single class of outage consumes more than 20% of error budget over a quarter, the team must have a P0 item on their quarterly planning document[^2] to address the issues in the following quarter.

### Escalation Policy

In the event of a disagreement between parties regarding the calculation of the [error budget](https://sre.google/workbook/slo-engineering-case-studies/) or the specific actions it defines, the issue should be escalated to the CTO to make a decision.
